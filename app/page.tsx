...
import { Send, Terminal, CheckCircle, XCircle, Clock, Plus } from "lucide-react"
...
export default function HomePage() {
  ...
  useEffect(() => {
    const ws = new WebSocket("ws://hqasvc-alb.bigbasket.com/bbkernos/ws/chat")
    wsRef.current = ws
    ...
    ws.onmessage = (event) => {
      try {
        ...
        const systemMessage: Message = {
          id: generateRandomId(8),
          content: data.content || "Received response",
          role: "system",
          timestamp: new Date(),
  ...
  const createNewConversation = () => {
    const newConversationId = generateRandomId(6)
    const starterMessage: Message = {
      id: generateRandomId(8),
      content: "How can I help you with your operations today?",
      role: "system",
      timestamp: new Date(),
    }
    setConversation({
